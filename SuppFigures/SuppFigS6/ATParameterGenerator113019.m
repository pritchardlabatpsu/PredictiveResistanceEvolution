...
n = 30; % number of resistance variants

             % CA  CG  CT  TA  TC  TG
pyr_biases = [.01 .01 .18 .003 .05 .009];
pyr_subs = {'CA' 'CG' 'CT' 'TA' 'TC' 'TG'};
pyr_idx = randsample(1:6,n,'true');
bias = pyr_biases(pyr_idx);
rho = bias/sum(bias);
...
csvwrite('ATParameters113019.csv',[rho' rel_alpha])
csvwrite('PyrSubs113019.csv',pyr_subs(pyr_idx)');