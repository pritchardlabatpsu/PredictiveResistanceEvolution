...
q = -1.00/30.4;        % Fassoni 2018 - PLSC net growth (with drug) [/day]
...
csvwrite('CumMutRisk.csv',[t; x_S; F_A; F_B]'); % saved for mu = 4e-8 and M = 1e-7

% figure
% plotidx = t<10*365;
% plot(t(plotidx)/365,log10(x_S(plotidx))/log10(M))
% hold on
% plot(t(plotidx)/365,[F_A(plotidx); F_B(plotidx)]) % note: scaled so cdfs are visible
% hold off
...
csvwrite('InstMutRisk.csv',[t; x_S; fstar_A; fstar_B]'); % saved for mu = 4e-8 and M = 1e-7
...
xlim([min(x) max(x)]/365)
ylim([min(x) max(x)]/365)
hold on
...
xlabel('t_{A}')
ylabel('t_{B}')
zlabel('Joint Conditional PDF')
xticks([6.5 7.5 T/365])
xticklabels({'6.5' '7.5' 'T'})
yticks([6.5 7.5 T/365])
yticklabels({'6.5' '7.5' 'T'})
