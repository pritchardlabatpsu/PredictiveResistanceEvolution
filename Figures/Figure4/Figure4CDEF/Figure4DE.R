...
setwd("C:/Users/Scott/Box Sync/MutBias/Reresubmit/Figures/Figure4/Figure4DE/")
...
drug = "Nilotinib"
abs.lim = max(summary.df[summary.df$drug==drug,c("sim.pred","clin.prev")],na.rm=T)

abs.fit.wbias = lm(sim.pred~clin.prev,data=summary.df[summary.df$drug%in%c(drug)&summary.df$bias==T,])
m.abs.wbias = abs.fit.wbias$coefficients[2]
b.abs.wbias = abs.fit.wbias$coefficients[1]

abs.fit.wobias = lm(sim.pred~clin.prev,data=summary.df[summary.df$drug%in%c(drug)&summary.df$bias==F,])
m.abs.wobias = abs.fit.wobias$coefficients[2]
b.abs.wobias = abs.fit.wobias$coefficients[1]

ggplot(summary.df[summary.df$drug%in%c(drug)&summary.df$bias==T,],aes(x=clin.prev,y=sim.pred))+theme_bw()+
  geom_abline(slope=1,color="gray75",size=2,linetype=2)+
  geom_abline(slope=m.abs.wbias,intercept=b.abs.wbias,color="red",size=2)+
  geom_point(color="black",size=5,alpha=.8)+
...
# ggsave("ImatAbsWBias.png",height=6,width=6)

ggplot(summary.df[summary.df$drug%in%c(drug)&summary.df$bias==F,],aes(x=clin.prev,y=sim.pred))+theme_bw()+
  geom_abline(slope=1,color="gray75",size=2,linetype=2)+
  ...
  xlim(0,abs.lim)+ylim(0,abs.lim)
# ggsave("ImatAbsWoBias.png",height=6,width=6)


abs.pears.wbias = cor(summary.df$clin.prev[summary.df$drug%in%c("Imatinib")&summary.df$bias==T],
                      summary.df$sim.pred[summary.df$drug%in%c("Imatinib")&summary.df$bias==T],method="pearson")
# 0.84

abs.pears.wobias = cor(summary.df$clin.prev[summary.df$drug%in%c("Imatinib")&summary.df$bias==F],
                       summary.df$sim.pred[summary.df$drug%in%c("Imatinib")&summary.df$bias==F],method="pearson")
# 0.21
